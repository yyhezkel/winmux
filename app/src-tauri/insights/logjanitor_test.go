...
// rotateCopyTruncate must copy oversized content to .1 and truncate the
// original in place (so an open O_APPEND fd keeps working).
func TestRotateCopyTruncate(t *testing.T) {
	...
	rotateCopyTruncate(p, 1024) // cap below current size → rotate
	...
	if err != nil || len(rotated) != len(big) {
		t.Fatalf(".1 should hold the old contents (%d bytes), got %d err=%v", len(big), len(rotated), err)
	}

	// Under the cap → no-op (no .1 churn).
