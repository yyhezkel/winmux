...
- **Status:** 5 MUST closed in Phase 35 (`bddc0b0`), auto port forwarding closed in Phase 36 / 36.A (`95de6f1`). Pipe hardening landed in Phase 44 (`c65b0c5`). Phase 48 closed several SHOULD/COULD items: #1.4 Ctrl+Alt+Arrow split-or-move, #3.3 ADR + threat-model docs, #4.9 /doctor diagnostic endpoint, #4.10 frontend stall instrumentation, plus BiDi 33A (HTML-surface tokens). Phase 49 closed: #2.3 drag-drop into terminal, #4.3 auto-destroy empty workspaces, #4.5 git worktree per workspace, #4.6 quadrant split keymap, #4.7 CHANGELOG postmortem habit (lives in CONTRIBUTING.md instead). 2 MUST remain (Secrets Vault deferred → external MCP integration, Full LLM control on the roadmap). Still on the inventory: 2.4 Diff pane, 3.1 lib.rs split, BiDi 33B (PTY-stream FSI/PDI). As individual items are decided, move them to their own Decided / Open entries with phase + commit references. Master inventory stays here until fully triaged.
